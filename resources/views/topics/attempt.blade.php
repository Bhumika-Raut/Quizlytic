    ...
    <div class="px-4 py-5 sm:px-6 flex justify-between items-start">
        <div>
            ...
            <p class="mt-1 max-w-2xl text-sm text-gray-400">
                You are taking a random 5-question quiz from the "{{ $topic->name }}" topic.
            </p>
        </div>
...
<form action="{{ route('topics.attempts.store', $topic) }}" method="POST">
    @csrf
    <input type="hidden" name="question_ids" value="{{ $questions->pluck('id')->join(',') }}">
    ...
    <div class="mt-8 flex justify-end">
        @if($questions->isEmpty())
            <div class="text-gray-400 italic p-6 bg-gray-800 border border-gray-700 rounded-xl w-full text-center shadow-lg">
                This topic currently has no questions. Check back later!
            </div>
        @else
